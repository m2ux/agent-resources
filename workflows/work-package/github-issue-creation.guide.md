...
[Additional information that helps understand the problem]
- Prior discussions or related issues
...
```markdown
# Add Diagram Awareness

## Problem Statement

The system processes PDF and EPUB documents to extract text-based chunks and 
concepts. However, technical documents often contain diagrams, flowcharts, and 
figures that convey information not captured in text. This visual content is 
currently lost during ingestion, reducing the knowledge base's completeness.

**Current state:**
- Documents are chunked as text segments only
- Diagrams are ignored or produce garbled OCR artifacts
- Search results cannot surface visual content
- Users cannot discover documents based on diagram content

**Desired state:**
- Diagrams and visual content are preserved during ingestion
- Visual content is searchable and discoverable
- Search results can be enriched with relevant diagrams
- Visual information contributes to concept understanding

## Goal

Enable users to discover and leverage visual content alongside text when 
searching their document library, enriching search results with visually 
...
- Editable/interactive diagrams

## User Stories

### US-1: Diagram Extraction
> As a user, I want diagrams extracted from my documents during ingestion 
> so that visual information is preserved in my knowledge base.

**Acceptance Criteria:**
- [ ] Diagrams, charts, and figures are detected in PDF documents
- [ ] Each visual is stored with its source document and page location
- [ ] Visual metadata includes type classification (diagram, chart, figure, table)
- [ ] Extraction works for both native PDFs and scanned documents

### US-2: Diagram Search
> As a researcher, I want to search for diagrams by topic so that I can 
> find visual explanations of concepts.

**Acceptance Criteria:**
- [ ] Diagrams are searchable via semantic search
- [ ] Search results include relevant diagrams alongside text chunks
- [ ] Results show document source, page number, and diagram type
- [ ] Results are ranked by relevance to query

### US-3: Visual Type Filtering
> As a user, I want to filter search results by visual type so that I can 
> find specific kinds of diagrams.

**Acceptance Criteria:**
- [ ] Visuals are classified by type: diagram, chart, figure, table
- [ ] Search can be filtered to specific visual types
- [ ] Filter is optional—default includes all types

### US-4: Visual-Enriched Results
> As a knowledge worker, I want search results enriched with related diagrams 
> so that I get both textual and visual context.

**Acceptance Criteria:**
- [ ] Text search results can optionally include associated visuals
- [ ] Visual relevance is based on semantic similarity to query
- [ ] Users can toggle visual enrichment on/off

## Success Metrics

| Metric | Target |
|--------|--------|
| Diagram detection recall | >80% of diagrams in test corpus detected |
| Search relevance | Diagrams appear in top-10 results when relevant |
| User satisfaction | >75% of visual results rated "helpful" |

## Constraints

- Must not significantly increase ingestion time (target: <50% overhead)
- Storage growth must be manageable (target: <10MB average per document)
- Must work with existing document formats (PDF, EPUB)
...
- [Architecture Review Guide](architecture-review.guide.md) - For documenting design decisions (comes after issue)
